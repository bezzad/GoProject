...
namespace GoProject
{
    public static class GoHelper
    {
        ...
        public static Node GetTrueTypeNode(this INode node)
        {
            ...
            {
                case NodeCategory.Pool: return new PoolNode(node);
                case NodeCategory.Lane: return new LaneNode(node);
                case NodeCategory.subprocess: return new SubProcessNode(node);
                case NodeCategory.@event: return new EventNode(node);
                case NodeCategory.activity: return new ActivityNode(node);
                case NodeCategory.gateway: return new GatewayNode(node);
                default: return new DataNode(node);
            }
        }
        ...
        public static List<Node> ConvertToTreeNodes(this List<Node> nodes)
        {
            ...
            foreach (var node in trueTypes)
            {
                ...
                {
                    // find node parent:
                    var parent = trueTypes.FirstOrDefault(x => string.Equals(node.Group, x.Key, StringComparison.InvariantCultureIgnoreCase) && x.IsGroup == true);
